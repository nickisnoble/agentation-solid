  ...
  return (
    <div style={{ height: "100%", position: "relative" }}>
      <svg width="100%" height="100%" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" fill="none">
        ...
        <circle cx={width * 0.3} cy={height * 0.3} r={Math.min(width, height) * 0.08} fill="var(--agd-fill)" stroke="var(--agd-stroke)" stroke-width="0.8" />
      </svg>
    </div>
  );
}
  ...
  return (
    <div style={{ height: "100%", display: "flex", "flex-direction": "column" }}>
      <div style={{ display: "flex", "border-bottom": "1px solid var(--agd-stroke)", padding: "6px 0" }}>
        <For each={Array.from({ length: cols }, (_, i) => i)}>
          {(i) => (
            <div style={{ flex: 1, padding: "0 8px" }}><Bar w="70%" h={3} strong /></div>
          )}
        </For>
      </div>
        ...
        {(r) => (
          <div style={{ display: "flex", "border-bottom": "1px solid rgba(255,255,255,0.03)", padding: "6px 0" }}>
            <For each={Array.from({ length: cols }, (_, c) => c)}>
              {(c) => (
                <div style={{ flex: 1, padding: "0 8px" }}><Bar w={`${50 + ((r * 7 + c * 13) % 40)}%`} h={2} /></div>
              )}
            </For>
          </div>
        )}
...
function AvatarSkeleton({ width, height }: SkeletonProps) {
  ...
  return (
    <svg width="100%" height="100%" viewBox={`0 0 ${width} ${height}`} fill="none">
      <circle cx={width / 2} cy={height / 2} r={r - 1} stroke="var(--agd-stroke)" fill="var(--agd-fill)" stroke-width="1.5" stroke-dasharray="3 2" />
      <circle cx={width / 2} cy={height * 0.38} r={r * 0.28} stroke="var(--agd-stroke)" fill="var(--agd-fill)" stroke-width="0.8" />
      <path
...
function ChartSkeleton({ width, height }: SkeletonProps) {
  ...
  return (
    <div style={{ height: "100%", display: "flex", "align-items": "flex-end", "justify-content": "space-around", padding: "0 4px", "border-bottom": "1px solid var(--agd-stroke)" }}>
      <For each={Array.from({ length: bars }, (_, i) => i)}>
        {(i) => {
          const h = 30 + ((i * 37 + 17) % 55);
          return <Block w={barW} h={`${h}%`} radius={2} />;
        }}
      </For>
    </div>
  );
      ...
      <For each={Array.from({ length: stars }, (_, i) => i)}>
        {(i) => (
          <svg width={sz} height={sz} viewBox="0 0 16 16" fill="none">
            <path d="M8 1.5l2 4 4.5.7-3.25 3.1.75 4.5L8 11.4l-4 2.4.75-4.5L1.5 6.2 6 5.5z" stroke="var(--agd-stroke)" stroke-width="0.8" fill={i < 3 ? "var(--agd-bar)" : "none"} />
          </svg>
        )}
      </For>
    </div>
...
function CheckboxSkeleton({ width, height }: SkeletonProps) {
  ...
  return (
    <svg width="100%" height="100%" viewBox={`0 0 ${width} ${height}`} fill="none">
      <rect x="1" y={(height - sz + 2) / 2} width={sz - 2} height={sz - 2} rx={sz * 0.15} stroke="var(--agd-stroke)" stroke-width="1.5" />
      <path d={`M${sz * 0.25} ${height / 2}l${sz * 0.2} ${sz * 0.2} ${sz * 0.3}-${sz * 0.35}`} stroke="var(--agd-bar)" stroke-width="1.5" fill="none" stroke-linecap="round" stroke-linejoin="round" />
    </svg>
  );
}
...
function SpinnerSkeleton({ width, height }: SkeletonProps) {
  ...
  return (
    <svg width="100%" height="100%" viewBox={`0 0 ${width} ${height}`} fill="none">
      ...
      <path d={`M${width / 2} ${height / 2 - r}a${r} ${r} 0 0 1 ${r} ${r}`} stroke="var(--agd-bar-strong)" stroke-width="1.5" stroke-linecap="round" />
    </svg>
  );
}
...
function ContactSkeleton({ width, height }: SkeletonProps) {
  return (
    <div style={{ height: "100%", display: "flex", "flex-direction": "column", padding: width * 0.04, gap: height * 0.03 }}>
      ...
      <Block w={Math.min(120, width * 0.3)} h={Math.min(30, height * 0.1)} radius={6} style={{ "align-self": "flex-end", background: "var(--agd-bar)" }} />
    </div>
  );
}
